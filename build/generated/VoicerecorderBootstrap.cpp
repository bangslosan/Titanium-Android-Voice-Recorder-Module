...
#define TAG "com.hybridtechja.voice"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Voicerecorder_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Voicerecorder.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::VoicerecorderBindings::lookupGeneratedInit(
...
static void Voicerecorder_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Voicerecorder_getBinding)->GetFunction());
}

static void Voicerecorder_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::VoicerecorderBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry VoicerecorderBinding = {
	"com.hybridtechja.voice",
	Voicerecorder_init,
	Voicerecorder_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_com_hybridtechja_voice_VoicerecorderBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.hybridtechja.voice", &VoicerecorderBinding);
	titanium::KrollBindings::addExternalLookup(&(::VoicerecorderBindings::lookupGeneratedInit));
}
